...
struct Segment {
  ...
  /// Initialize a segment with head, tail, and angle
  /// - Parameters:
  ///   - head: The head position
  ///   - tail: The tail position
  ///   - angle: The rotation angle in radians
  init(head: SIMD2<Float>, tail: SIMD2<Float>, angle: Float) {
    self.head = head
    self.tail = tail
  ...
  init(head: SIMD2<Float>, tail: SIMD2<Float>, cosAngle: Float, sinAngle: Float, negativeCosAngle: Float, negativeSinAngle: Float) {
    self.head = head
    self.tail = tail
  ...
  /// Create a segment with a specified head position, angle, and length
  /// - Parameters:
  ///   - head: The head position
  ...
  init(
    ...
    self.init(head: head, tail: tail, angle: angle)
  }
  ...
  func duplicate(translation: SIMD2<Float>) -> Segment {
    Segment(head: head + translation, tail: tail + translation, cosAngle: cosAngle, sinAngle: sinAngle, negativeCosAngle: negativeCosAngle, negativeSinAngle: negativeSinAngle)
  }
}
